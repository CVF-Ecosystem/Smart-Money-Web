      ...
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div className="login-card">
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <button type="button" className="btn btn-ghost" style={{ width: '100%', justifyContent: 'center', fontSize: 14 }} onClick={() => alert('Chưa hỗ trợ Google Login trong phiên bản hiện tại')}>
  ...
  async function processRecurringOnLogin() {
    try {
      ...
      if (created && created.length > 0) {
        ...
        setTimeout(() => {
          alert(`🔄 Đã tự động tạo ${created.length} giao dịch định kỳ`);
        }, 1000);
      }
    } catch (error) {
